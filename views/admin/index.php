...
<div id="app-refresh" style="font-family:'Segoe UI',Roboto,Arial,sans-serif; background:radial-gradient(circle at top,#0b1220,#05070f); height:100vh; display:flex; flex-direction:column; color:#e5e7eb; overflow:hidden;">
    
    <div style="position: sticky; top: 0; background: #05070f; z-index: 30; padding: 30px 30px 10px 30px; flex-shrink: 0;">
        <div style="max-width:1200px; margin:0 auto; background:rgba(15,23,42,0.92); border:1px solid rgba(255,255,255,0.08); border-radius:24px 24px 0 0; padding:28px 28px 10px 28px; border-bottom: none;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:25px;">
                ...
                <div style="display:flex; gap:10px; align-items:center; margin-left:auto;">
                    ...
                    <div id="notificationBell" style="position:relative; padding:8px 12px; background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.08); border-radius:10px; font-size:13px; cursor:pointer;">
                        ...
                        <?php endif; ?>
                        <div id="notificationDropdown" style="display:none; position:absolute; right:0; top:calc(100% + 12px); width:340px; max-height:370px; overflow:auto; background:rgba(15,23,42,0.98); border:1px solid rgba(255,255,255,0.1); box-shadow:0 18px 50px rgba(0,0,0,0.35); border-radius:18px; z-index:40; padding:12px;">
            ...
            <div style="display:flex; gap:15px; flex-wrap:wrap; margin-bottom:15px;">
                <div style="flex:1; min-width:180px; background:rgba(59,130,246,0.12); border:1px solid rgba(59,130,246,0.2); border-radius:14px; padding:12px 16px;">
                    <div style="font-size:13px; color:#93c5fd;">Total Tickets</div>
                    <div style="font-size:24px; font-weight:800; margin-top:4px;">
                        <?= count($tickets) ?>
                    </div>
                </div>
                <div style="flex:1; min-width:180px; background:rgba(245,158,11,0.12); border:1px solid rgba(245,158,11,0.2); border-radius:14px; padding:12px 16px;">
                ...
                <div style="flex:1; min-width:180px; background:rgba(34,197,94,0.12); border:1px solid rgba(34,197,94,0.2); border-radius:14px; padding:12px 16px;">
    ...
    <div style="flex: 1; overflow-y: auto; padding: 0 30px 10px 30px; min-height: 0;">
        <div style="max-width:1200px; margin:0 auto; background:rgba(15,23,42,0.92); border-left:1px solid rgba(255,255,255,0.08); border-right:1px solid rgba(255,255,255,0.08); padding:0 28px 20px 28px;">
            
            <div style="background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.08); border-radius:16px; overflow:auto; backdrop-filter:blur(10px);">
                <table style="width:100%; border-collapse:collapse; min-width:1000px;">
                    <thead>
                        <tr style="background:rgba(255,255,255,0.04); text-align:left; position: sticky; top: 0; z-index: 10;">
                            <th style="padding:14px; background:#111927;">ID</th>
                            <th style="padding:14px; background:#111927;">Title</th>
                            <th style="padding:14px; background:#111927;">Description</th>
                            <th style="padding:14px; background:#111927;">Created By</th>
                            <th style="padding:14px; background:#111927;">Status</th>
                            <th style="padding:14px; background:#111927;">Created</th>
                            <th style="padding:14px; background:#111927;">Assigned Agent</th>
                            <th style="padding:14px; text-align:center; background:#111927;">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                                ...
                                <td style="padding:14px; text-align:center; white-space:nowrap;">
                                    <?php if($t['status'] !== "closed"): ?>
                                        ...
                                        <a href="../tickets/ticket-details.php?id=<?= $t['id'] ?>" style="background:#10b981; color:white; padding:6px 10px; border-radius:8px; text-decoration:none; font-size:12px; font-weight:600; margin-right:6px;">
                                            View<?= ($t['unread_count'] > 0) ? ' <span style="color:#ff0033; font-weight:900; font-size:13px; margin-left:4px;">' . $t['unread_count'] . '</span>' : '' ?>
                                        </a>
                                    <?php else: ?>
                                        <span style="color:#22c55e; font-weight:700; font-size:13px; margin-right:6px;">
                                            Resolved
                                        </span>
                                    <?php endif; ?>
    ...
    <div style="position: sticky; bottom: 0; background: #05070f; z-index: 30; padding: 0 30px 30px 30px; flex-shrink: 0;">
        <div style="max-width:1200px; margin:0 auto; background:rgba(15,23,42,0.92); border:1px solid rgba(255,255,255,0.08); border-radius:0 0 24px 24px; padding:15px 28px; text-align: center; font-size: 12px; color: #64748b; border-top: none;">
            © <?= date('Y') ?> Admin Ticket Dashboard Support System. All rights reserved.
        </div>
    </div>
        ...
        async function refresh() {
            ...
            try {
                ...
                if (newContainer && current) {
                    ...
                    // Re-apply event handling on newly injected element variants
                    const bell = document.getElementById('notificationBell');
                    const dropdown = document.getElementById('notificationDropdown');
                    if(bell && dropdown) {
